...
class StudyGuideGenerator(Workflow):
    ...
    def __generate_study_guide(self, topic: str) -> study_guide_planner.StudyGuide:
        try:
            ...
            study_guide: study_guide_planner.StudyGuide = self.study_guide_agent.run(
                topic
            ).content
            ...
            try:
                ...
                study_guide: study_guide_planner.StudyGuide = (
                    self.study_guide_agent.run(topic).content
                )
                logger.info("Study Guide Generation Finished...")
                return study_guide
            except Exception:
    ...
    def run(self, topic: str) -> Iterator[RunResponse]:
        ...
        yield RunResponse(event="STUDY_GUIDE", content=study_guide.dict())
        ...
        self.session_state["session"]["study_guide_confirmation_msg"] = (
            study_guide_confirmation_msg
        )
        yield RunResponse(
            event="AUDIO_TRANSCRIPT", content=study_guide_confirmation_msg
        )

...
class DeepResearcher(Workflow):
    ...
    def __generate_tldraw_items(self, report: Report) -> List[Dict]:
        ...
        intro_item = {
            ...
            "contents": [
                {
                    "type": "sticky",
                    "text": f"Background: {report.introduction.background}",
                },
                {
                    "type": "sticky",
                    "text": f"Objective: {report.introduction.objective}",
                },
            ],
            "position": {"x": 500, "y": 50},
    ...
    def run(
        self, topic: str, researcher: GPTResearcher, report
        ...
        yield RunResponse(event="RESEARCH_REPORT", content=json.dumps(report))

        if current_research.get("report_summary", None):
            resport_gen_msg = current_research["report_summary"]
        else:
            resport_gen_msg = self.__generate_confirmation_msg(
                f"Write a short 100 word summary for the report. Report: {report}"
            )
            current_research["report_summary"] = resport_gen_msg
        # send report summary for voice
        yield RunResponse(event="AUDIO_TRANSCRIPT", content=resport_gen_msg)
        ...
        yield RunResponse(event="RESEARCH_CONTEXT", content=json.dumps(context))
        ...
        yield RunResponse(event="RESEARCH_SOURCES", content=json.dumps(sources))
        ...
        yield RunResponse(event="RESEARCH_IMAGES", content=json.dumps(images))
        ...
        tl_draw_items = self.__generate_tldraw_items(report)
